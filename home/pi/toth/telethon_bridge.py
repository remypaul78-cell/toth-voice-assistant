...
class TothTelegramBridge:
    ...
    async def _async_run(self):
        ...
        async def handler(event):
            ...
            # VERIFICATION DYNAMIQUE: lire le fichier a chaque message
            
            if os.path.exists("/tmp/toth_vocal_all"):
                if not self._vocal_all:
            ...
            if self._vocal_all:
                ...
                _tech = ("💻", "📖", "📚", "⚙️", "🔧", "⚡", "💾", "⚠️", "🔍", "🌐", "📝", "📋", "🔗", "🎬", "🎨", "🔊", "[[audio", "MEDIA:", "⏳", "👁️", "📸", "🎯", "🚀", "🤖", "📊", "✅", "❌", "🎵", "🎸", "🎙️", "🛠️", "📡", "🔔", "💬", "📨", "📜", "🏷️", "📌", "📍", "🧠", "💭", "🧩", "⏰", "🗓️", "⏸️", "▶️", "🔄", "⬆️", "⬇️", "🟢", "🔴", "🟡", "📦", "🧹", "# write_file", "# ", "```", "import ", "from PIL", "from toth", "write_file", "# write_file", "patch", "ORDRE #", "ORDRE ", "execute_code", "from hermes_tools", "import terminal", "ssh pi@", "scp ", "sudo systemctl", "✏️")
                ...
                if text.strip().startswith(_tech) or any(p.lower() in text.lower() for p in _err):
                    ...
                    return
                # Si Toth enregistre (bouton appuye), queue sans TTS
                
                if os.path.exists("/tmp/toth_recording"):
                    print(f"[TELETHON] VOCAL TOTAL (recording, queue): {text[:60]}")
